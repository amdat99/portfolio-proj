...
export function* fetchItemsPending() {
  yield takeLatest(shopActionTypes.FETCH_ITEMS_PENDING, fetchItemsAsync);
}

export function* fetchCategoryPending() {
  yield takeLatest(
    shopActionTypes.FETCH_CATEGORY_PENDING,
    fetchCategoriesAsync
  );
}

export function* fetchProductPending() {
  yield takeLatest(shopActionTypes.FETCH_PRODUCT_PENDING, fetchProductAsync);
}

export function* fetchFilteredItemsPending() {
  yield takeLatest(
    shopActionTypes.FETCH_FILTEREDITEMS_PENDING,
...
export function* fetchSellingItemsPending() {
  yield takeLatest(
    shopActionTypes.FETCH_SELLINGITEMS_PENDING,
    fetchSellingItemsAsync
  );
}
