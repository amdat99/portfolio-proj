...
export function* uploadProfileImageAsync({
  payload: { profileImage, profileId },
}) {
  try {
    const profileImageData = yield call(
      ...
      profileImage,
      profileId
    ); // passes in firestore collection name
    yield put(fetchProfileImageSuccess(profileImageData));
  } catch (error) {
    yield put(fetchProfileImageFailed(error.message));
  }
...
export function* onFetchProfileImagePending() {
  yield takeLatest(
    profileActionTypes.FETCH_PROFILEIMAGE_PENDING,
    fetchProfileImageAsync
  );
}

export function* onFetchProfileInfoPending() {
  yield takeLatest(
    profileActionTypes.FETCH_PROFILEINFO_PENDING,
