          ...
          <ul id={"message-box-container"}>
            {currentUser ? (
              onProfileImage && userid !== currentUser.profileId.toString() ? (
                ...
                <img
                  id="message-box-profile"
                  src={`https://firebasestorage.googleapis.com/v0/b/aamir-project-492ef.appspot.com/o/images%2Fprofile${userid}?alt=media&token=b54a3d9a-0bac-44b8-9035-717aa90cb4e6`}
                  alt="profile"
                  height="34"
                  width="30"
                  onError={() => setOnProfileImage(false)}
                />
              ) : null
            ) : onProfileImage ? (
              <img
          ...
          {currentUser ? (
            onVid && userid === currentUser.profileId.toString() ? (
              <ReactPlayer
                url={video}
                controls
                width={vidReady ? "200px" : "0px"}
                height={vidReady ? "150px" : "0px"}
                onError={vidToggle}
